...
class InfoGAN(object):
    ...
    def discriminator_logits(self, input_batch):
        prev_layer = input_batch
        for layer_i in xrange(len(self.discriminator_params['dim'])):
            prev_layer_size = prev_layer.get_shape().as_list()[1]
            cur_kernel = self.all_weights['disc_w'+str(layer_i)]
            ...
            if layer_i+1==len(self.discriminator_params['dim']):
                nonlin = tf.nn.bias_add(conv, self.all_weights['disc_b'+str(layer_i)])
            else:
                #linbn = tf.nn.bias_add(conv, self.all_weights['disc_b'+str(layer_i)])
                linbn = self.batch_norms['disc_bn' + str(layer_i)](conv)
                nonlin = tf.nn.elu(linbn)
                prev_layer = nonlin
        logits = tf.reshape(nonlin, [-1, 2])
        return logits

    def get_losses(self):
        self.fake_images = self.generator()
        fake_disc_logits = self.discriminator_logits(self.fake_images)
        real_disc_logits = self.discriminator_logits(self.images)
